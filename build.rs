...
fn main() {
    ...
    build
        ...
        .define("BUILD_INFO", "\"(smartmon crate)\"")
...
fn config_defines(target: &Target) -> Vec<(&'static str, String)> {
    ...
    let mut d = vec![
        ...
        ("SMARTMONTOOLS_CONFIGURE_ARGS", q(" [smartmon crate]")),
...
fn write_config_h(path: &Path, defines: &[(&str, String)]) {
    let mut s = String::from("/* Generated by smartmon build.rs */\n");
    for (k, v) in defines {
        s += &format!("#define {k} {v}\n");
    }
...
fn write_smartmon_config_h(path: &Path, defines: &[(&str, String)]) {
    let mut s = String::from(
        "/* Generated by smartmon build.rs */\n\
